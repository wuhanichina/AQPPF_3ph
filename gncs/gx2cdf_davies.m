function [p,flag]=gx2cdf_davies(x,w,k,lambda,m,s,varargin)
	...
	addParameter(parser,'MaxIntervalCount',200000,@(x) isreal(x) && isscalar(x) && (x>0));
	
	parse(parser,x,w,k,lambda,m,s,varargin{:});
	side=parser.Results.side;
	AbsTol=parser.Results.AbsTol;
	RelTol=parser.Results.RelTol;
	MaxIntervalCount=parser.Results.MaxIntervalCount;
	...
	% compute the integral (single execution path: use quadgk with explicit tolerances)
	davies_integral=arrayfun(@(x0) quadgk(@(u) davies_integrand(u,x0-m,w',k',lambda',s), ...
		0,inf,'AbsTol',AbsTol,'RelTol',RelTol,'MaxIntervalCount',MaxIntervalCount),x);
